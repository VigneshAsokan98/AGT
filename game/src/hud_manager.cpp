...
void hud_manager::init()
{
	...
	m_HealthBar = Billboard::create("assets/textures/healthBar.png", 1, 1, 1);
...
void hud_manager::on_update(const engine::timestep& time_step, const engine::perspective_camera& camera)
{
	...
	m_HealthBar->activate(healthBarPos, m_player_health * .05f, .005f);
...
{
	m_HealthBar->on_render(camera, shader);
